...
//	    	  Toast.makeText(getBaseContext(), ((TextView)v.findViewById(R.id.lngValue)).getText().toString(),4).show();  	  
	    	  ...
	    	  int req_id=Integer.parseInt(((TextView)v.findViewById(R.id.imageID)).getText().toString());
	    	  String imagename=((TextView)v.findViewById(R.id.imageName)).getText().toString();
	    	  String status=((TextView)v.findViewById(R.id.imageStatus)).getText().toString();
	    	  
	    	  if(status.compareToIgnoreCase("ready")==0){
	    	  String[] conn_data = TestOBA.oba.getConnectData(req_id);
	    	  
	    	  Toast.makeText(getBaseContext(), conn_data.toString(), Toast.LENGTH_LONG);
	    	  ...
	    	  {
	    		  Toast.makeText(getBaseContext(), "Please wait till your reservation is ready", Toast.LENGTH_LONG).show();
	    	  }
	    	  else if(status.compareToIgnoreCase("timedout")==0)
	    	  {
	    		  Toast.makeText(getBaseContext(), "Your reservation has timed out", Toast.LENGTH_LONG).show();
	    	  }
...
ArrayList<Map<String,String>> list = build();
	...
	public ArrayList<Map<String, String>> build() {
	    ...
	    for (Object object : req_id) {
	    	...
	    	HashMap req_status= TestOBA.oba.getRequestStatus(Integer.parseInt((String) obaImagesHash.get("requestid")));
	...
	public void conn_do_ssh(String[] conn_data_secure, String imagename){
		...
		i++;
		try {
			String Conn_URI="ssh://"+conn_data_secure[1]+ "@"+conn_data_secure[0]+":22/#"+imagename;
		...
		//Show password in notification bar
			Context context = this
				...
				//Start Connect BOT
				//TODO enclose with try catch and handle catch prompting user to install connectBOT
			try{Intent intent = new Intent("android.intent.action.VIEW", Uri.parse(Conn_URI));
		    startActivity(intent);}
			catch (android.content.ActivityNotFoundException ex) {
			    Toast.makeText(getBaseContext(), "Please install connectBot", Toast.LENGTH_LONG).show();
			}
...
//	intent.setComponent(new ComponentName("com.toremote.serversmanager", 	"com.toremote.RemoteActivity")); 
	 //for standard version 
	//intent.setComponent(new ComponentName("org.toremote.serversmanager", 
	// "com.toremote.RemoteActivity")); 
	//for lite version 
	try{intent.setComponent(new ComponentName("org.toremote.rdpdemo", 
	 "com.toremote.RemoteActivity"));
	}
	catch(ActivityNotFoundException e){
		Toast.makeText(getBaseContext(), "Please install REMOTE RDP from the Android Market to be able to remotely connect to this Reservation", Toast.LENGTH_LONG).show();
	}
